...
let nuevaNoticia = unaNoticia => {
    return `
        <article class="${unaNoticia.tipoNacional ? "x" : "y"}">
            <small class="adloremipsum">${unaNoticia.fecha}</small>
            <h3 class="sarasa">${unaNoticia.titulo}</h3>
            <p>${unaNoticia.descripcion}</p>
            <img src="${unaNoticia.imgUrl}" alt="${unaNoticia.titulo}">
        </article>
    `
}

window.addEventListener("load", () => {
    let contenedorNacionales = document.querySelector("#nacionales");
    let contenedorInternacionales = document.querySelector("#internacionales");
    noticias.forEach( noticia => {
        if(noticia.tipoNacional) {
            contenedorNacionales.innerHTML += nuevaNoticia(noticia)
        } else {
            contenedorInternacionales.innerHTML += nuevaNoticia(noticia)
        }
    })
})